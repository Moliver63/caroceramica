...
checkoutRouter.post("/", async (req, res) => {
  try {
    ...
    // Validação básica do payload — evita crash por acesso a propriedade
    // de undefined quando o client envia dados incompletos/malformados.
    if (
      !body?.cliente?.nome ||
      !body?.cliente?.email ||
      ...
      return res.status(400).json({
        erro: "Dados do pedido incompletos. Verifique cliente, endereço e itens.",
      });
    }
    ...
    // Integração Asaas — se falhar (chave inválida, gateway fora do ar, etc.),
    // o pedido já foi registrado como "aguardando_pagamento" no banco; o erro
    // é reportado ao cliente sem derrubar o servidor.
    try {
    ...
    console.error("Erro ao processar checkout:", erro);
    return res.status(500).json({
      erro: "Erro interno ao processar o pedido. Tente novamente em instantes.",
    });
  }
});
